...
export function SubjectFormSheet({
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<SubjectValues>({ resolver: zodResolver(subjectSchema(tv)), defaultValues: EMPTY });

  useEffect(() => {
    if (!open) return;
    reset(subject ? { name: subject.name, code: subject.code, series: subject.series } : EMPTY);
  }, [open, subject, reset]);
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select value={field.value} onValueChange={field.onChange}>
                      <SelectTrigger id="series" className="w-full"><SelectValue /></SelectTrigger>
                      <SelectContent>
